...
def _call_groq_with_retry(fn, max_retries=2, base_delay=1.5):
    """
    Call `fn()` (a zero-arg callable wrapping a client.chat.completions.create
    call) with retry-with-backoff on rate limits / transient errors.
    Raises LLMUnavailable with a friendly message if all attempts fail.
    """
    last_err = None
        ...
        try:
            ...
            if status == 429:
                ...
                raise LLMUnavailable(
                    "The AI service is rate-limited right now (too many requests). "
            ...
            raise LLMUnavailable(
                "The AI service rejected the request. Please rephrase the question "
                "or try again later."
            ) from e
            ...
            if attempt < max_retries:
                ...
                continue
            raise LLMUnavailable(
                "Could not reach the AI service (network/timeout). Please try again."
            ) from e
        except Exception as e:
            # Catch-all so a single bad response never crashes the app with a
            # raw traceback in front of a judge.
            last_err = e
            raise LLMUnavailable(
    ...
    raise LLMUnavailable("The AI service did not respond.") from last_err
...
def parse_question_with_llm(question):
    ...
    return _call_groq_with_retry(_do_call)
...
def llm_answer_question_cached(question, subject_id):
    """
    Session-scoped cache so unrelated Streamlit reruns (e.g. expanding an
    expander, or any other widget interaction elsewhere on the page) never
    re-fire the same two LLM calls for a question that was already answered
    for this patient in this session. This is the fix for the rate-limit
    failure caused by redundant reruns — behavior/answers are unchanged.
    """
    if "qa_cache" not in st.session_state:
        st.session_state.qa_cache = {}

    cache_key = (subject_id, question.strip().lower())
    if cache_key in st.session_state.qa_cache:
        return st.session_state.qa_cache[cache_key]

    result = llm_answer_question(question, subject_id)
        ...
        if client is None:
            ...
            try:
                with st.spinner("Parsing question and retrieving evidence..."):
                    out = llm_answer_question_cached(question, subject_id)
